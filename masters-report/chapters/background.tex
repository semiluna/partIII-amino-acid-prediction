...
Formally, for a given graph $\mathcal{G} = (\mathcal{V}, \mathcal{E})$, at message passing iteration $k$, a hidden embedding $\textbf{h}_u^{(k)}$ corresponds to each node $u \in \mathcal{V}$, and an update happens according to information aggregated from $u$'s graph neighbourhood $\mathcal{N}(u)$, as seen in Equation \ref{message-passing}.
\begin{align}
    \textbf{h}_u^{(k+1)} &= \text{UPDATE}^{(k)}(\textbf{h}_u^{(k+1)}, \text{AGGREGATE}^{(k)}(\{\textbf{h}_v^{(k+1)},\forall v\in \mathcal{N}(u)\}))\\ 
    &=\text{UPDATE}^{(k)}(\textbf{h}_u^{(k+1)},\textbf{m}^{(k)}_{\mathcal{N}(u)})
\label{message-passing}
\end{align}
Note that in general the $\text{AGGREGATE}$ function must be permutation-invariant. 
